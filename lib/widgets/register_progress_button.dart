...
class RegisterProgressButton extends StatefulWidget {
  final String text;

  const RegisterProgressButton({Key key, this.text}) : super(key: key);

  @override
  _RegisterProgressButtonState createState() => _RegisterProgressButtonState();
}

class _RegisterProgressButtonState extends State<RegisterProgressButton>
  ...
  Future<void> animateButton() async {
    ...
    if (await Get.find<SignupController>().signUp()) {
      setState(() {
        state = 2;
      });
    } else {
